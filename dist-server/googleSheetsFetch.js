...
export function parseSpreadsheetIdAndGid(input) {
...
export async function fetchPublicGoogleSheetAsCsv(urlOrId) {
    ...
    const res = await fetch(exportUrl, {
        headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; ShutuAnalysis/1.0; +https://localhost) AppleWebKit/537.36 (KHTML, like Gecko)',
            Accept: 'text/csv,*/*',
        },
        redirect: 'follow',
    });
    ...
    if (head.includes('<!doctype') || head.includes('<html')) {
        throw new Error('未获取到 CSV（可能为登录页）。请将表格共享为「知道链接的任何人可查看」或使用「发布到网络」后再试。');
    }
    if (!text.trim()) {
