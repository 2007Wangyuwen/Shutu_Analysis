...
function extractInputCsvFromPrompt(text) {
    const m = text.match(/【输入内容】\s*([\s\S]*)$/);
    return (m?.[1] || '').trim();
}
function inferColumns(csvText) {
    const lines = csvText.split(/\r?\n/).filter((l) => l.trim());
    if (!lines.length)
        return [];
    return lines[0].split(',').map((s) => s.trim()).filter(Boolean);
}
function buildBasicModeResult(messages) {
...
function buildBasicModeSchemesOnly(messages) {
    const latestUser = [...messages].reverse().find((m) => m.role === 'user')?.content || '';
    const csvText = extractInputCsvFromPrompt(latestUser);
    const columns = inferColumns(csvText);
    const c1 = columns[0] || '类别';
...
app.post('/api/ecnu/chat', async (req, res) => {
...
app.get('/api/chat', async (req, res) => {
    ...
    try {
        ...
        if (!canUseDeepseek) {
            const basic = session.chartPart === 'schemes-only'
