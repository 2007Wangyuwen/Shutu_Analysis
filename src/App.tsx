...
${dataInput}`;
  ...
  const exportText = () => {
    ...
    a.href = url;
    a.download = '术图_数据参考分析报告.md';
    a.click();
    URL.revokeObjectURL(url);
  };

  /** 为导出图像追加底部署名条（长图与 PDF 共用） */
  const appendExportSignature = (source: HTMLCanvasElement): HTMLCanvasElement => {
    const footerHeight = Math.max(28, Math.min(44, Math.round(source.height * 0.035)));
    ...
    ctx.lineTo(output.width, source.height + 0.5);
    ctx.stroke();

    const padX = Math.max(12, Math.round(output.width * 0.02));
    const fontPx = Math.max(11, Math.min(14, Math.round(footerHeight * 0.4)));
    ctx.font = `500 ${fontPx}px "Inter", "PingFang SC", "Microsoft YaHei", sans-serif`;
    ctx.fillStyle = 'rgba(26,26,26,0.58)';
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
    ctx.fillText('Generated by 术图 Shutu Analysis', output.width - padX, source.height + footerHeight / 2);
    return output;
  };
  ...
  /** 与导出长图/PDF 共用：仅截取「数据参考分析报告」面板为 canvas */
  const captureAnalysisPanelToCanvas = async (): Promise<HTMLCanvasElement | null> => {
    const root = reportPanelRef.current;
    if (!root) return null;

    const replaceUnsupportedColorFunctions = (input: string): string => {
      ...
      while (i < input.length) {
        ...
        const chIdx = rest.indexOf('oklch(');
        const abIdx = rest.indexOf('oklab(');
        let relIdx = -1;
        if (chIdx >= 0 && abIdx >= 0) relIdx = Math.min(chIdx, abIdx);
        else relIdx = Math.max(chIdx, abIdx);
        if (relIdx < 0) {
          out += input.slice(i);
          break;
        }
    ...
    const captured = await html2canvas(root, {
      ...
      logging: false,
      onclone: (clonedDoc) => {
        const hasUnsupportedColorFn = (v: string) => /oklch|oklab/i.test(v);
        const safeValueForProp = (prop: string): string => {
          const p = prop.toLowerCase();
          if (p.includes('background-image')) return 'none';
          if (p.includes('shadow')) return 'none';
          if (p.includes('background')) return '#f5f2ed';
          if (
            p.includes('color') ||
            p.includes('fill') ||
            p.includes('stroke') ||
        ...
        clonedDoc.querySelectorAll('*').forEach((el) => {
          ...
          if (s) {
            el.setAttribute('style', replaceUnsupportedColorFunctions(s));
          }
        });

        // 2. 清除所有 <style> 标签内的 oklch / oklab
        clonedDoc.querySelectorAll('style').forEach((st) => {
          st.textContent = replaceUnsupportedColorFunctions(st.textContent || '');
        });

        // 3. 兜底：清洗计算样式里仍残留的 oklch / oklab，避免 html2canvas 解析时报错
        ...
        clonedDoc.querySelectorAll<HTMLElement>('*').forEach((el) => {
          ...
          for (let i = 0; i < computed.length; i += 1) {
            ...
            if (!value || !hasUnsupportedColorFn(value)) continue;
            el.style.setProperty(prop, safeValueForProp(prop), 'important');
          }
        });
      },
    });
    return appendExportSignature(captured);
  };
          ...
          <div className="lg:col-span-6 xl:col-span-6 flex flex-col gap-4 min-h-0 min-w-0 lg:h-[calc(100vh-7.5rem)] lg:overflow-y-auto">
            ...
            {displayParsed.options.length > 0 && (
              <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700" ref={analysisPanelRef}>
                ...
                {/* Intro Text */}
                {displayParsed.intro && (
                  <div ref={reportPanelRef} className={`premium-panel p-4 sm:p-8`}>
